...
2. **Preconditions** — requires `nono`, `git`, and the target tool to be on PATH (`require_cmd`); requires being run from the root of a git repository (not a subdirectory) since `.nono_ignore` resolution depends on repo-relative paths.
3. **Profile generation** (`build_profile`) — turns `.nono_ignore` (gitignore syntax) into a per-project, per-tool nono profile at `.nono/<tool>.profile.json`, which `extends` the tool's base nono profile. It merges two inputs: `.nono_ignore` → `filesystem.deny`, and `.nono/<tool>.extra.json` (see step 5) → `filesystem.deny/allow/read/write`. `allow`/`read`/`write` entries win over a conflicting `.nono_ignore` deny for the same path (set subtraction), so a path explicitly allowed via the save-profile flow isn't left denied. `.nono_ignore` itself and the `.nono/` state dir are always denied so the sandboxed agent can't read or edit its own rules. `build_profile` also appends `.nono/` to the project's `.gitignore` (creating it if needed, via `ensure_gitignore_has_state_dir`) — this state dir is never meant to be committed.
   - `compute_deny_paths` shells out to `git ls-files --ignored --exclude-from=.nono_ignore --directory` to get git's own gitignore-pattern matching (negation, `**`, anchoring) instead of reimplementing it.
   - Profile rebuild is skipped unless the combined sha256 of `.nono_ignore` and `.nono/<tool>.extra.json` (cached in `.nono/<tool>.ignore.sha256`) has changed (`needs_rebuild`), so unrelated runs stay cheap.
...
Key invariant: `.nono/<tool>.profile.json` (and the schema/hash files alongside it) are local, disposable state (`.nono/` is auto-gitignored, see step 3) — never hand-edit them or treat them as source of truth; they're regenerated from `.nono_ignore` and `.nono/<tool>.extra.json`. `.nono_ignore` is the human-owned source of truth; `.nono/<tool>.extra.json` is leash-owned generated state (written only by `append_extra`) for grants `.nono_ignore` can't express — it's still meant to be hand-edited when revoking a previously-absorbed grant (remove the path from its array), just not treated as authoritative the way `.nono_ignore` is.
